...
namespace GendouDumper
{
    ...
    public class WebReq
    {
            ...
            string sFilename, bool bReturnStr)
        {
            ...
            outHeaders.Add("User-Agent: ToxWR/1.0.0 (Windows NT 5.1; U; en)");
            ...
            try
            {
                ...
                if (!ParseHeader())
                {
                    ...
                    if (Headers["Location"] != null)
                        sNewURL = "_<loc>" + Headers["Location"] + "</loc>";

                    if (ResponseCode != "") throw new Exception("#02-0004_" + ResponseCode + sNewURL);
                ...
                while ((nBytes = socket.Receive(RecvBuffer, 0, cSize, SocketFlags.None)) > 0)
                {
                    ...
                    if (ReturnStr) Response += Encoding.Unicode.GetString(RecvBuffer, 0, nBytes);
                }
                ...
                if (File.Exists(tFName))
                {
                    if (ReturnStr)
                    {
                        FileStream fsRet = new FileStream(
                            tFName, FileMode.Open, FileAccess.Read);
                        byte[] b = new byte[fsRet.Length];
                        fsRet.Read(b, 0, (int)fsRet.Length);
                        Response = System.Text.Encoding.UTF8.GetString(b);
                        fsRet.Flush(); fsRet.Close(); fsRet.Dispose();
                    }
                    if (Filename != "")
                    {
                        File.Delete(Filename); File.Move(tFName, Filename);
                    }
                    else File.Delete(tFName);
                }
                else
                {
        ...
        private bool ParseHeader()
        {
            try
            {
                ...
                if (matches.Count > 0)
                {
                    ...
                    if (matches[0].Value.Contains(" 302 ")) ResponseCode = "302";
                    if (matches[0].Value.Contains(" 301 ")) ResponseCode = "301";
                }
